...
def generate_combined_mask(
    rgb_image, model, frame_index=None, device='cuda:0', imgsz=(480, 640), conf=0.5, first_frame=False, output_json_path=None
):
    """
    ...
    Args:
        ...
        output_json_path (str, optional): The path to save the JSON file if `first_frame` is True.
    ...
    if first_frame:
        ...
        # Save JSON if a path is provided
        if output_json_path:
            with open(output_json_path, "w") as f:
                json.dump(json_data, f, indent=4)
            print(f"JSON saved to {output_json_path}")

        return combined_mask, segmentation_time, json_data

    # If not first_frame, return only the combined mask and segmentation time
    return combined_mask, segmentation_time


# Plot the binary mask with bounding box
def plot_mask_with_bbox(rgb_image, binary_mask, bbox):
    """Plots the binary mask and bounding box on the image."""
    plt.figure(figsize=(10, 10))

    # Overlay the binary mask on the original image
    plt.imshow(cv2.cvtColor(rgb_image, cv2.COLOR_BGR2RGB), alpha=0.7)
    plt.imshow(binary_mask, cmap="jet", alpha=0.3)
    ...
    plt.title("Binary Mask with Bounding Box")
    plt.axis("off")
    plt.show()


# Load the YOLO model (replace with your model path)
model_path = "/home/martyn/Thesis/YOLOv8/axle_seg.pt"
seg_model = YOLO(model_path)

# Path to your test image
image_path = "/home/martyn/Thesis/pose-estimation/data/scenes/scenes_axle/scenes_single/scene_09/rgb.png"
rgb_image = cv2.imread(image_path)

# Call the function to generate the combined mask and bounding box
output_json_path = "test_output.json"
combined_mask, segmentation_time, json_data = generate_combined_mask(
    rgb_image=rgb_image,
    model=seg_model,
    frame_index=1,
    first_frame=True,
...
# Extract the bounding box from the JSON data
bbox = json_data[0]["bbox"]

# Plot the results
plot_mask_with_bbox(rgb_image, combined_mask, bbox)

# Print information and confirm JSON saved
