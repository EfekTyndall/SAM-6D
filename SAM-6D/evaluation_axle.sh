...
export CAD_PATH="/home/martyn/Thesis/pose-estimation/data/cad-models/sdit01888D53e5s6_Meshed_Decimated_Scaled.ply"
...
BASE_OUTPUT_DIR="/home/martyn/Thesis/pose-estimation/results/axle/methods/sam6d/"

# The root directory containing the 10 scenes
SCENES_ROOT="/home/martyn/Thesis/pose-estimation/data/scenes/scenes_axle/scenes_single/"
...
for scene_num in $(seq 1 $NUM_SCENES); do
  ...
  for run_idx in $(seq 1 $NUM_RUNS); do
    ...
    export OUTPUT_DIR
    export SEG_PATH=$SCENE_DIR/manual_detection.json

    # Go to the SAM-6D directory and run the pipeline
    cd "${SAM6D_DIR}"

    cd Pose_Estimation_Model
    python run_pem_eval_axle.py --output_dir $OUTPUT_DIR --cad_path $CAD_PATH --rgb_path $RGB_PATH --depth_path $DEPTH_PATH --cam_path $CAMERA_PATH --seg_path $SEG_PATH
  done
done
